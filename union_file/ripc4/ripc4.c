...
void set_key(workspace_t *ws) {
  ...
  /*
  enc_state= range(256)
  target = shellcode + range(256)(not in shellcode)
  for i in range(256):
    #find j
    ini_idx = enc_state.index(target[i])
    key_data[i] = 0x100 - state[i]
  */
...
void set_input(workspace_t *ws) {
  ...
  ws->buf_len = strlen(ws->input_buf);//off-by-one?
}
